...
async function handlerPATCH(
  ...
  // Todo el update (pedido + item + sync con el taller) es atómico:
  // si algo falla no queda el pedido a medio actualizar.
    ...
    if (d.estado) {
      ...
      if (d.estado === "PARA_FABRICAR") {
        ...
        if (noTerminadas.length) {
          await tx.espejoFabricacion.updateMany({
        ...
        if (d.estado === "ENTREGADO") {
          ...
          if (espejoIds.length) {
            await tx.espejoCatalogo.updateMany({
        ...
        if (sinEmpezar.length) {
          await tx.espejoFabricacion.deleteMany({
